...
const std::vector<element_t> collect_relivant_files(const std::vector<element_t>& directory, const std::vector<element_t>& extns_to_delete)
{
	...
	try 
	{
		for (auto& dir_iter : directory)
		{
			...
			{
				//std::unique_ptr<std::vector<element_t>> file_path_p(new std::vector<element_t>);
				...
				if (!fs::is_directory(*file_path_p))
				{
					...
					{	
						...
						{
							if (file_path_p->extension() == i)
							//significant reduction in ram (110%) just by changing this to look for specific ext. types instead of during organization of nodes.
							{ 
								vfiles->push_back(dir);
								std::cout << "File:\t" << dir << '\n';
							}
						}
					}
