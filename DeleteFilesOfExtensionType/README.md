...
a delete version, that deletes files of a type(s) specific to multimedia, made to delete images that can be used for blackmail secretly (WIP), use the branch: DeleteProgram, will be cross compatable as well in the future. 

TODO: delete from the nodes directly after organizing, no need to run the loop now, we only have nodes we need to delete the extensions - instead of a linked list of all the extensions as nodes from the drives. . 
      


---

not yet tested on LinuxOS, kali, tested on windows (x64):
...
to run:
    ...
    [from compiled directory]./FileExtensionTypeOrganizer.out <source (directory location to copy from)>  
